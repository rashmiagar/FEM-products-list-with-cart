    ...
    <div className="p-4 bg-white w-[25%] rounded-lg ml-4">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex justify-between">
          Order Total{" "}
          <h2 className="text-2xl text-yellow-950 font-extrabold">
            ${cart.total.toFixed(2)}
          </h2>
        </div>
        <button
          className={`bg-amber-800 rounded-2xl p-2 w-[70%] ml-[15%] text-white cursor-pointer ${cart.items.length === 0 ? "opacity-50 cursor-not-allowed" : ""}`}
      ...
      {isVisible && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 shadow-lg min-w-[40%] relative">
            {/* <button
            ...
            <img src="/assets/images/icon-order-confirmed.svg" className="w-10 h-10 mx-auto mb-4 ml-0" alt="Order Confirmed" />
            <h3 className="text-3xl font-bold">Order Confirmed</h3>
            <p className="text-stone-500 font-light text-sm">We hope you enjoy your food!</p>
           <div className="bg-[#FCF8F5FF] p-4 mt-4">
            <div>
              {cart.items.map((item) => {
                return (
                  <div
